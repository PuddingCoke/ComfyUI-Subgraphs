...
&emsp;&emsp;HED预处理图像和LineArt预处理图像有时候会少一点东西，前两张图中的左手就没对应上。可以在子节点左边的"差分图像"节点中，利用Mask Editor，使用它自带的吸管和橡皮擦工具选择想添加的缺失结构来创建遮罩，并使用合成图像（遮罩）节点来添加缺失结构。以下是采取上述措施的两张示例图。
